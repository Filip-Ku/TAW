...
const posts = [
    ...
    },
    {
        id: 10,
        title: 'Post 10',
        text: 'Bufallo chicken boudin leberkas. Shoulder spare ribs tongue chicken drumstick tenderloin venison pancetta pork salami hamburger rump cupim alcatra meatloaf. Turkey chuck beef ribs porchetta, cupim beef pancetta kevin tri-tip tongue swine leberkas strip steak pastrami. ',
...
const dataEndpoint = (router) => {
    ...
    response.status(200).send({post: posts.splice(startIndex,limit)});
    });
   ...
   router.get('/api/posts', async (request, response, next) => {
       ...
       if (titleSearch) {
           ...
           if (filteredPosts.length === 0) {
               return response.status(404).send("Post nie znaleziony.");
           }
       }
    ...
    router.post('/api/posts/',async (request,response,next) => {
        const {title,text,image} = request.body.newPost


        if (title==null && text==null && image==null) {
            posts.push(request.body.newPost);
            response.status(200).send({post: posts[request.params.id-1]});
        } else {
            response.status(400).send("Uzupełni dane")
        }
    });
    ...
    router.delete('/api/posts/:id', async (request,response,next) => {
        if(request.params.id<posts.length-1 && request.params.id>0) {
    ...
    router.put('/api/post/:id',async(request,response,next)=> {
        ...
        const newPost = request.body.newPost;
        posts[postId-1]=newPost
        response.send({post: posts[request.params.id-1]})
    });
};
